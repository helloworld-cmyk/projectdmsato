...
(() => {
  ...
  style.textContent += ".matchup-chat-close{padding:0;line-height:0}.matchup-chat-close>.material-symbols-rounded{width:24px;height:24px;min-width:0;overflow:hidden;display:grid;place-items:center;line-height:1;text-align:center}.matchup-chat-typing{display:flex;align-items:flex-end;gap:7px;max-width:86%}.matchup-chat-typing-avatar{width:27px;height:27px;display:grid;place-items:center;flex:0 0 auto;border-radius:50%;color:#fff;font-size:8px;font-weight:800}.matchup-chat-typing-copy{display:flex;flex-direction:column;gap:3px}.matchup-chat-typing-name{margin-left:3px;color:#7b8b82;font-size:9px;font-weight:800}.matchup-chat-typing-bubble{display:inline-flex;align-items:center;gap:3px;width:max-content;padding:9px 11px;border-radius:13px 13px 13px 4px;background:#fff;box-shadow:0 2px 9px rgba(27,61,39,.06)}.matchup-chat-typing-bubble i{width:5px;height:5px;border-radius:50%;background:#75a184;animation:matchup-chat-typing 1s infinite ease-in-out}.matchup-chat-typing-bubble i:nth-child(2){animation-delay:.15s}.matchup-chat-typing-bubble i:nth-child(3){animation-delay:.3s}@keyframes matchup-chat-typing{0%,60%,100%{opacity:.35;transform:translateY(0)}30%{opacity:1;transform:translateY(-3px)}}";
  ...
  const renderChat = () => {
    ...
    chatLayer.querySelector("[data-chat-subtitle]").textContent = `${Math.max(2, (match.participants || []).length)} thành viên · ${access.status === "pending" ? "đang chờ duyệt" : "đã duyệt"}`;
    ...
    const typingMessage = [...messages].reverse().find((message) => message.kind !== "system" && message.senderId !== "self");
    const typingParticipant = (match.participants || []).find((player) => player.name !== store.getProfile().name) || (match.participants || []).find((player) => player.role === "Chủ kèo") || {};
    const typingMember = typingMessage ? { name: typingMessage.senderName, initials: typingMessage.senderInitials, tone: typingMessage.senderTone } : { name: typingParticipant.name || "Minh Khang", initials: typingParticipant.initials || "MK", tone: typingParticipant.tone || "#6680ba" };
    const typingMarkup = typingMatchCounts.has(activeChatMatchId) ? `<div class="matchup-chat-typing" role="status" aria-live="polite"><span class="matchup-chat-typing-avatar" style="background:${escape(typingMember.tone || "#6680ba")}">${escape(typingMember.initials || "MU")}</span><div class="matchup-chat-typing-copy"><span class="matchup-chat-typing-name">${escape(typingMember.name || "Thành viên")} đang nhập</span><span class="matchup-chat-typing-bubble" aria-label="Đang nhập"><i></i><i></i><i></i></span></div></div>` : "";
    ...
    }).join("") : '<div class="matchup-chat-system">Hãy là người đầu tiên chào cả đội.</div>'}${typingMarkup}`;
